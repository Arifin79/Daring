...
public class CreateTaskBottomSheetFragmentTeacher extends BottomSheetDialogFragment {
    ...
    TodoFragmentTeacher fragment = new TodoFragmentTeacher();
    ...
    public void setTaskId(int taskId, boolean isEdit, setRefreshListener setRefreshListener, TodoFragmentTeacher
            fragment) {
        this.taskId = taskId;
        this.isEdit = isEdit;
    ...
    public void setupDialog(@NonNull @NotNull Dialog dialog, int style) {
        ...
        View contentView = View.inflate(getContext(), R.layout.bottom_sheet_create_todo, null);
    ...
    public boolean validateFields() {
        if(addTaskTitle.getText().toString().equalsIgnoreCase("")) {
            Toast.makeText(fragment.getContext(), "Please enter a valid title", Toast.LENGTH_SHORT).show();
            return false;
        }
        else if (addTaskDescription.getText().toString().equalsIgnoreCase("")){
            Toast.makeText(fragment.getContext(), "Please enter a valid description", Toast.LENGTH_SHORT).show();
            return false;
        }
        else if (taskDate.getText().toString().equalsIgnoreCase("")){
            Toast.makeText(fragment.getContext(), "Please enter date", Toast.LENGTH_SHORT).show();
            return false;
        }
        else if (taskTime.getText().toString().equalsIgnoreCase("")){
            Toast.makeText(fragment.getContext(), "Please enter time", Toast.LENGTH_SHORT).show();
            return false;
        }
        else if(taskEvent.getText().toString().equalsIgnoreCase("")) {
            Toast.makeText(fragment.getContext(), "Please enter an event", Toast.LENGTH_SHORT).show();
            return false;
        }
        else {
            return true;
    ...
    private void createAnAlarm() {
        try {
            ...
            Intent alarmIntent = new Intent(fragment.getActivity(), AlarmBroadcastReceiver.class);
            ...
            PendingIntent pendingIntent = PendingIntent.getBroadcast(fragment.getActivity(), count, alarmIntent, PendingIntent.FLAG_UPDATE_CURRENT);
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                PendingIntent intent = PendingIntent.getBroadcast(fragment.getActivity(), count, alarmIntent, 0);
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
